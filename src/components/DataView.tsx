...
const DataView = ({ accounts, usdPln }: DataViewProps) => {
  const rate = usdPln ?? 3.7; // fallback if rate is not available yet
    ...
    <div className="w-full max-w-full flex-1 min-h-100 sm:min-h-110 lg:min-h-120 lg:max-h-screen h-full rounded-3xl border border-white/10 bg-linear-to-br from-white/10 via-white/5 to-white/10 p-4 sm:p-6 lg:p-7 shadow-[0_25px_80px_rgba(0,0,0,0.55)] backdrop-blur-xl flex flex-col gap-6 lg:gap-7 overflow-hidden">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <h2 className="text-2xl sm:text-3xl font-semibold text-white">Data View</h2>
        </div>
        <div className="w-full sm:w-auto rounded-xl border border-white/15 bg-linear-to-br from-black/10 via-white/6 to-black/20 px-4 py-3 shadow-[0_10px_22px_rgba(0,0,0,0.28)] flex items-center justify-between gap-4 backdrop-blur">
          <div className="leading-tight">
            <p className="text-[10px] uppercase tracking-[0.18em] text-white/55">Total balance</p>
            <div className="text-xl sm:text-2xl font-semibold text-white tracking-tight">{formatPln(total)}</div>
            <p className="text-[11px] text-white/55 mt-1">Last updated: {lastUpdatedText}</p>
          </div>
          <span className="flex h-10 w-10 sm:h-9 sm:w-9 items-center justify-center rounded-full border border-white/15 bg-white/10 shadow-[0_6px_14px_rgba(0,0,0,0.3)]">
      ...
      <div className="flex-1 flex flex-col gap-5">
        <div className="flex-1 w-full rounded-3xl border border-white/10 bg-white/10 p-2.5 sm:p-3.5 shadow-inner">
          <div className="w-full h-55 sm:h-65 md:h-77.5 lg:h-full lg:min-h-80">
